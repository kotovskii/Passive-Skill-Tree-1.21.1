...
public class TooltipHelper {
    ...
    public static MutableComponent getSkillTitle(@NotNull PassiveSkill skill) {
        ...
        if (skill.getTitle().isEmpty()) {
            ...
            if (title.getString().equals(descriptionId)) {
                title = Component.literal(idToName(skillId.getPath()));
            }
        } else {
    ...
    public static Component getRecipeTooltip(ResourceLocation recipeId) {
        ...
        List<AbstractWorkbenchRecipe> recipes = recipeManager.getAllRecipesFor(PSTRecipeTypes.WORKBENCH.get()).stream().map(recipe -> recipe.value()).toList();
