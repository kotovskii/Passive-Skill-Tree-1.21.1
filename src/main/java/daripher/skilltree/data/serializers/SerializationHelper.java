...
public class SerializationHelper {
    ...
    public static @Nullable MobEffect deserializeMobEffect(JsonObject json) {
        ...
        if (effect == null) {
            throw new JsonParseException("Mob effect " + effectId + " doesn't exist!");
        }
        return effect;
    }
    ...
    public static MobEffectInstance deserializeEffectInstance(JsonObject json) {
        ...
        return new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(Objects.requireNonNull(effect)), duration, amplifier);
    }
    ...
    public static ResourceLocation deserializeModifierId(String id) {
        if (id.contains(":")) {
            return ResourceLocation.parse(id);
        }
        String path = id.toLowerCase(Locale.ROOT).replace("-", "_").replaceAll("[^a-z0-9/._-]", "_");
